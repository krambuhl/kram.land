...
The mode named `dark` is the colour scheme and is the only mode with four scopes; `inverted` has no meaning for a mode that is not two-valued. Any other mode, such as `contrast`, gets an OS media block that fills its slot plus `[data-<mode>='on']` and `[data-<mode>='off']` to force it. A token with several modes chains its slots in config order with the base last.
...
`token-jet export --dtcg` writes the token tree in the DTCG format's first stable version, 2025.10. That version fixes the value shapes: a colour is an object, `{ colorSpace, components, alpha?, hex? }`, and a dimension is `{ value, unit }` with only `px` and `rem` allowed. `$type`, `$description` and `$deprecated` map directly. References are `{path}`. Modes mirror the config under `$extensions['token-jet'].modes`: the media queries at the file root, and each token's mode values on the token. The resolver module's `modifiers` and `contexts` vocabulary was considered and passed over: the config's modes are flat named keys, and mapping them onto an axis with contexts would invent a `light` context and a fake axis for a mode such as `contrast`. `token-jet import --dtcg <file>` reads the same format into a `tokens.config.ts` skeleton of modes and tokens; `types` and `contrast` are not in the format and are added by hand.
...
- Responsive token props, such as `p={{ base: token('space.x4'), sm: token('space.x8') }}`, are deferred. Every mechanism found has a multiplier. Enumerated classes are property × breakpoint × token, and pruning them to used combinations requires the whole argument object to be syntactically literal at every level, which the type system cannot enforce (`const key = 'md'` still has type `'md'`) and which forbids a component from forwarding a responsive prop. Inline custom properties avoid the class count but make the atomic unit `{ className, style }` instead of a class string, and inline `style` is blocked by a CSP without `'unsafe-inline'`. Responsive behaviour stays in CSS Modules until a real need on the site chooses between those costs.
