...
Only safe commits land on `main`. A safe commit leaves the site working: `npm run lint`, `npx tsc --noEmit` and `npm run build` all pass at that commit, not just at the end of the series.
